...
std::vector<DungeonUtility::BlockData*>& DungeonUtility::StructureObject::getBlocks() {
    return this->blocks;
}
...
export(jlongArray, dungeonutility_struct_StructureObject_getBlocks, jlong pointer){
    std::vector<DungeonUtility::BlockData*>& blocks = ((DungeonUtility::StructureObject*) pointer)->getBlocks();
    jlongArray array = env->NewLongArray(blocks.size());
    //env->SetLongArrayRegion(array, 0, blocks.size(), blocks.data());
    return array;
}
